...
{
  s_fifo_size_out.write(s_region_fifo_out.num_available());
  
  if(s_region_fifo_out.num_free() > 0) {
    s_busy_out.write(false);
    
    PixelData p = matrix.readPixelRegion(region_num, time_now);
    ...
    if(mClusteringEnabled) {
      if(mClusterStarted == false) {
        ...
        if(p == NoPixelHit) { // Indicates that we already read out all pixels from this region          
          if(mPixelHitmap == 0)
            s_region_fifo_out.write_nb(AlpideDataShort(mPixelHitEncoderId, mPixelHitBaseAddr));
          else
            s_region_fifo_out.write_nb(AlpideDataLong(mPixelHitEncoderId, mPixelHitBaseAddr, mPixelHitmap));
      ...
      if(p == NoPixelHit) {
        ...
        s_region_fifo_out.write_nb(AlpideDataShort(encoder_id, base_addr));
        s_region_empty_out.write(false);        
      }
    }
  } else { // FIFO is full
